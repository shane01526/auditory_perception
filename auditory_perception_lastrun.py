...
from psychopy import sound as ps_sound
...
expName = 'auditory_perception'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\user\\Desktop\\RA 事務\\2025新計畫\\拾玉計畫\\auditory_perception\\auditory_perception_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    blocks = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('conditions_blocks.xlsx'), 
        seed=None, 
        isTrials=False, 
    )
    ...
    for thisBlock in blocks:
        ...
        thisExp.currentRoutine = block_break
        ...
        while continueRoutine:
            ...
            # has the Routine been forcibly ended?
            if block_break.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in block_break.components:
        ...
        if break_key.keys in ['', [], None]:  # No response was made
            break_key.keys = None
        blocks.addData('break_key.keys',break_key.keys)
        if break_key.keys != None:  # we had a response
            blocks.addData('break_key.rt', break_key.rt)
            blocks.addData('break_key.duration', break_key.duration)
        ...
        for thisTrial in trials:
            ...
            sound_1 = ps_sound.Sound(sound1, sampleRate=44100, stereo=True, hamming=True)
            sound_2 = ps_sound.Sound(sound2, sampleRate=44100, stereo=True, hamming=True)
            ...
            while continueRoutine:
                ...
                if t >= 1.0 and not sound_1_played:
                    sound_1.play()
                    sound_1_played = True
                
                if t >= 2.5 and not sound_2_played:
                    sound_2.play()
                    sound_2_played = True
